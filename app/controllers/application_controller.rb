class ApplicationController < ActionController::Base
  ...
  http_basic_authenticate_with :name => "admin", :password => "security_subway"
  
  def check_for_mobile
    session[:mobile_override] = params[:jqmobi] if params[:jqmobi]
    prepare_for_mobile if mobile_device?
  end
  
  def prepare_for_mobile
    prepend_view_path Rails.root + 'app' + 'views_jqmobi'
  end
